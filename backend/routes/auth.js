...
router.post('/createuser',[
   ...
   if(user){
    return res.status(404).json({success, error:"a user whith this email is already exsist"})
   }
...
router.post('/login',[
    ...
    try{
        ...
        if(!user){
            success=false
           return res.status(404).json({success,error:"please enter the valid credential"})
        }
